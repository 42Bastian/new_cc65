...
struct frag {
  ...
  int f_value;                  /* value.  if literal, points to a
                                 * frag_literal.  */
...
struct frag *
add_frag(int type, int value, SYM * sym)
{
...
void
genbyte(int byte, int flags, SYM * sym)
{
...
void
genword(int word, int flags, SYM * sym)
{
...
void
gen_lbr(int basecode,           /* base opcode */
        SYM * sym)
{
#ifdef DEBUG
  ...
  /*
   * we know basecode is a byte.  we'll use the top byte later, as a flag to
   * say whether it's a long or short branch
   */
}
...
int
frag_gen_size(struct frag *this_frag, int frag_pc)
{
  ...
  case FRAG_LITERAL:
    {
      l = (struct frag_literal *)this_frag->fl_ptr;     /* n bytes this literal
                                                         * frag */
      return (l->nbytes);
    }
  case FRAG_BR_OFFSET:
  ...
  case FRAG_LONG_BR:
    {
      ...
      /*
       * this one's tricky.  if the symbol's defined, and in this module,
       * we're in good shape; just compute the offset from the symbol to the
       * module PC and mark the frag short or long, depending on the offset.
       * if the symbol's not defined yet, scan forward thru frags counting
       * sizes until find symbol or cumulative size gets too big.  If found
       * it within arbitrary offset, assume real offset will be short enough
       * to generate a short branch
       */
      ...
      if (sy->flags & THIS_SEG) {       /* this sym defined here? */
        ...
        else if ((this_frag->f_value & 0xff) != 0x80)   /* 42BS */
...
**********************************************************************/
void
assign_next_num(SYM * sym)
{
  /*
   * only count symbols that are a) declared global _and_ defined b) external
   * labels which are actually used here
   */
...
void
gen_o_open(char **name, char *source_name)
{
  char *ext;
