...
void
macro_expand(char *line, char *macro_line,
...
int DefineMacro(char *name){
  ...
  * p_m = (struct macroS *)macroHeapNext;
  ...
  while (!end_file && endm_notfound && macro_read_line(inf, line)) {
    ...
    size += p.opcode ? strlen(p.opcode) : 0;
    //4 - spaces

    for (i = 0; i < 32 && p.arg[i]; ++i)
      size += 1 + strlen(p.arg[i]);
    //strlen + ","
    ...
    if (endm_notfound) {
      ...
      if (p.label_suffix) {
        strcpy(ptr, p.label_suffix);
        ptr += strlen(p.label_suffix);
      }
      *ptr++ = ' ';
      if (p.opcode) {
        strcpy(ptr, p.opcode);
...
int do_macro() {
  ...
  if (verbose > 1)
    printf("%4d:expanding(%s)\n", line_nbr, p.opcode);

  if (!(m = CheckMacro(p.opcode)))
    return 0;
