...
/*error.c*/
extern void barf(char *msg, uintptr_t arg1, uintptr_t arg2, uintptr_t arg3);
/* xgen.c */
int gen_n_passes();
           ...
           int no_redef)
{
  ...
  if (sym_p->value != value && (pass != gen_n_passes() - 1)){
    barf("'%s' redefined: %lX vs %lX\n",
         (uintptr_t)name,
         (uintptr_t)sym_p->value,
         (uintptr_t)value);
  }
    ...
    for (sym_p = symtab[i]; sym_p;) {
      ...
      if ((sym_p->flags & USED) && !(sym_p->flags & DEFINED)) {
        ...
        if (sym_p->flags & LOCAL)
          barf("'%s' localy undefined (first use)", (uintptr_t)sym_p->name, 0, 0);
        else
          barf("'%s' undefined (first use)", (uintptr_t)sym_p->name, 0, 0);
        ++udef;
      }
      sym_p = sym_p->next;
    }
