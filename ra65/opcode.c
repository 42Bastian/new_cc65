...
#define OPBR    0x0800          /* like abs, but different so we can tell
                                 * what to generate */
...
/* given a name, return T if it's an opcode, and if so, return
   its code and flags */
int
opcode_p(char *name, int *code, int *valid)
{
  int i;
...
void
assemble_op(int basecode, int valid, char **args)
{
  ...
  case OPIMM:
    {
      ...
      default:
        /*
         * bit of a kludge here... gen code ^ 0x04, but if bit 0 is clear,
         * clear bit 3 as well
         */
        if (basecode & 0x01)
      ...
      *expr_flags = e_flags;
      ...
      if (arg1) {
        if (string_equal(arg1, "Y") || string_equal(arg1, "X")) {
          indirect_p = toupper(arg1[0]);
          /*
           * later, figure out whether to use short addressing; for now just
           * do it the easy way
           */
