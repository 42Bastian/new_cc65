...
int map_syms(int (*fun) (struct sym *, char *), char *funarg);
...
int search_lib(struct sym *sym, char * ignore);
...
int
open_carefully(char *name, int mode)
{
  int fd;
...
/* intern a symbol */
struct sym *
intern(char *str, int flags, int value)
{
  int hash;
  char *p;
  struct sym *sym;
...
int
reference(struct sym *sym, struct obj_module *m)
{
...
struct obj_module *
digest_object_file(char *name, int module)
{
  struct obj_module *m;
  int j, k;
...
int
count_undef_sym(struct sym *sym, char *arg)
{
  if (!(sym->flags & DEFINED))
    n_undef++;
  return (0);
...
int
sym_needed(struct sym *sym, char *sym_name)
{
...
/* for debugging */
int
describe_sym(struct sym *sym, char *str)
{
...
int
search_lib(struct sym *sym, char * ignore)
{
...
void
process_segdata(struct obj_module *mod, int segment)
{
  int j, k, op;
