...
*/
int
compile(int n_xmacro, char *xmacro[])
{
  /* char dummy; */
  int i;
...
*/
int
gettype(int dflt, struct hashent **sptr)
{
  int strtype;
  int sz;
  *sptr = NULL;
...
*/
int
declstruct(struct hashent *last, int strtype)
{
  int offset;
...
*/
void
ptype(struct hashent *psym, char *tarray)
{
  char *p;
  ...
  for (p = tarray; *p != '\0'; ++p) {
    if (*p & T_STRUCT) {
      /*
       * jrd here ... printf("struct/union of %s\n", ((struct hashent *
       * )decode(p))->name);
       */
