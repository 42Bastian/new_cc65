...
/* internal prototypes */
void needtok(uintptr_t token, char *msg);
void needcol();
void getoprnd(int (*func) (struct expent *), struct expent *lval);
...
static int kcalc(uintptr_t tok, int val1, int val2);
uintptr_t op_assoc(uintptr_t tok, struct op_alist *ops);
...
static void hie6_internal(int isptr1, struct expent *lval, void (*gen) (int));
...
*/
void
needtok(uintptr_t token, char *msg)
{
  if (curtok == token) {
    gettok();
    return;
...
*/
int
isconst(struct expent *lval1, struct expent *lval2, char *svptr)
{
...
*/
int
expr(int (*func) (struct expent *), struct expent *lval)
{
  int k;
  char *locptr;
  uintptr_t savsp;
  ...
  if (savsp != oursp) {
    ...
    sprintf(errbuf, "oursp [%lu] != savsp [%lu]", oursp, savsp);
    Error(errbuf);
  }
  return (k);
}
...
hieOr(struct expent *lval)
{
  ...
  if (curtok != DBAR) {         /* if not ||, return */
    ...
    loadk = ((lval->e_test & E_LOGL) == 0);     /* need to load 1 if not
                                                 * already */
    exprhs(k, lval);            /* get first expr */
    lab2 = getlabel();          /* get internal label.  for     tests, this is
                                 * the only label */
    ...
    while (curtok == DBAR) {    /* while there's more expr */
      ...
      loadk |= ((lval2.e_test & E_LOGL) == 0);  /* need to load 1 if not
                                                 * already */
      if (curtok == DBAR) {     /* if still more, ... */
        truejump(lab1, lval->e_test & E_XINV);  /* bne out */
      } else {
        if (loadk)              /* only generate branch around load if we're
                                 * loading */
...
static int
kcalc(uintptr_t tok, int val1, int val2)
{
  switch (tok) {
  case BAR:
    return (val1 | val2);
  ...
  default:
    printf("Internal error: kcalc: got token %lX\n", tok);
...
uintptr_t
op_assoc(uintptr_t tok, struct op_alist *ops)
{
  while (ops->tok && (ops->tok != tok))
    ++ops;
  return ((uintptr_t)ops->gen);
}

/* helper fun for various hie frobs */

int
hie_internal(struct op_alist *ops, struct expent *lval, int (*hienext) (struct expent *))
{
  /*
...
static void
hie6_internal(int isptr1, struct expent *lval, void (*gen) (int))
{
  /*
...
hie6(struct expent *lval)
{
  ...
  if ((curtok < LE) || (curtok > GT)) {
    ...
    lval->e_test = E_LOGL | E_CC;       /* say we returned a logl val, and
                                         * set cc */
    while (1) {
      switch (curtok) {
      case LE:
