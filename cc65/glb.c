...
parseinit(char *tptr)
{
  ...
  case T_ARRAY:
    ...
    if ((tptr[4] == T_CHAR) && (curtok == SCONST)) {
      ...
      count = strlen(str) + 1;
      /*
       * outdat(count); i = 0; while (++i <= count) outbyte(*str++);
       */
      outbv(str, count);
  ...
  default:
    if (tptr[0] & T_STRUCT) {
      ...
      q = (struct hashent *)(((uintptr_t)decode(tptr)) + (uintptr_t)gblspace);
