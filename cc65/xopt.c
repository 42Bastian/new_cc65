/*  optimizer for cc65
    ...
    97/12/08  42BS  had to remove the postinc/dec optimization because it
                    causes trouble with function-prototypes.
...
char *nextline( char *currentline);
bool matchstr( char *line,  char *str);
...
opt_asl()
{
  ...
  if (!matchjsr(line, "pushax"))
    return false;

  if (!matchstr(l1, "\tldax\t#"))
    return false;

  if (!matchjsr(l2, "asltos"))
    return false;

  if (!(isdigit(l1[7]) && (num = atoi(l1 + 7)) > 0))
    return false;
...
//
char *tos_func[] =
  {"multos", "divtos", "modtos", "or_tos", "xortos",
   "andtos", "addtos", "subtos", 0};
char *axy_func[] =
  {"mulaxy", "divaxy", "modaxy", "or_axy", "xoraxy", "andaxy",
   "addaxy", "subaxy", "aslaxy", "asraxy", 0};
...
opt_math2()
{
  ...
  if (i < 8)
    ...
    if (i == 8)
      //replace shifts by multyplication
      l3[1] = '*';
    else
      l3[1] = '/';
    //or division
...
opt_push()
{
  ...
  if (!isdigit(line[7]) || (num = atoi(line + 7)) > 255)
    return false;
...
main(int argc, char **argv)
{
  ...
  *(in_buf + in_len) = 0;       /* needed for CR LF => LF conversion ! */
  ...
  //here we go
  //
  int total_opt = 0, pass_opt = 0, opt = 0;

  struct opt_func *func;
  ...
  if (verbose && total_opt)
    printf("total          (%d)\n", total_opt);
  else if (verbose)
    printf("Nothing done on %s !\n", argv[argc]);
  //
...
writeldy(int n)
{
  LDY[HN] = dec2hex[(n >> 4) & 15];
  LDY[LN] = dec2hex[n & 15];
  writeLn(LDY);
}

void
writeconst(char *l, int n)
{
  l[HN] = dec2hex[(n >> 4) & 15];
  l[LN] = dec2hex[n & 15];
  writeLn(l);
