...
void lconst(int ctype, uintptr_t cnst);
void outconst(int ctype, uintptr_t cnst, int ischar);
void outconstprim(int ctype, uintptr_t cnst);
void test(int label);
void needbrack(uintptr_t btype);
...
void
cscale(struct expent *lval, int n)
{
  /*
   * if (lval->e_flags == E_MCONST) { lval->e_const *= n; rmvbyte(4);
   * lconst(E_TCONST, lval->e_const); } else {
   */
  scale(n);
  /* }       */
}
...
*/
void
lconst(int ctype, uintptr_t cnst)
{
  ctype &= E_MCTYPE;
  if (ctype == E_TLOFFS) {
    getladr(cnst);
...
void
outconstprim(int ctype, uintptr_t cnst)
{
...
void
outconst(int ctype, uintptr_t cnst, int ischar)
{
  switch (ctype & E_MCTYPE) {
  case E_TCONST:                /* fixnum constant */
    outdecnl(cnst);
...
*/
void
callfunction(int k, struct expent *lval)
{
  /* char * dptr; */
  struct expent lval2;
  int nargs;
...
*/
int
opref(int k, struct expent *lval)
{
...
*/
int
oppref(int k, struct expent *lval)
{
  char *tptr;
