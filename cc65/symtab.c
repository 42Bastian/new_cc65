...
*/
void
addsfld(struct hashent *psym, char *tarray, int offset,
        struct hashent *sn, struct hashent *son)
{
  char *tptr;
  //struct hashent * ps;
...
*/
int
addstag(struct hashent *psym, int sz)
{
...
*/
struct hashent *
findsfld(char *sym, struct hashent *parent)
{
  struct hashent *p;
...
*/
void
addglb(struct hashent *psym, char *tarray, int sc)
{
  char *tptr;
  int sz1;
...
*/
  ...
  psym->flag.g = sc;
  psym->tptr.g = tptr;
  psym->data.g = (uintptr_t)&psym->name;
...
*/
void
addloc(struct hashent *psym, char *tarray, int sc, int offs)
{
  char *tptr;
  ...
  /*
   * if ((tarray != NULL) && (tarray[0] == T_FUNC)) { addglb(psym, NULL,
   * SC_DEFINED | SC_GLOBAL); return; }
   */
  if (psym->flag.l != 0) {
